功能概要：基于MQXLite的KW01-PCNode(KW01-PC节点）数据透传功能程序
...
1. 工程概述
(1)本工程为基于MQXLite的KW01 PC节点数据透传功能测试样例程序，主要包括RF收发和串口收发
...
(3)若RF在1秒之内未接收到数据，则通过串口显示发送数据包内容，若接收成功，则发送接收成功数据包。

5. 数据帧格式设计
（1）PCNode本身接收的命令—握手命令
帧头   长度[硬件地址  +  数据]       硬件地址      数据       帧尾
1B  1B                     1B      nB   1B
P   1+n                    1B      nB   C
（2）PCNode需要转发的命令—转发命令
帧头   长度[硬件地址  +  数据]       硬件地址      数据        帧尾
1B  1B                     1B      nB   1B
P   1+n                    1B      nB   C
...
帧内数据格式(字节数 nB = 2B + 2B + 1B + mB)：
NetID   +  NodeID  +  Cmd  +  CmdContent
2B         2B         1B   +  mB
