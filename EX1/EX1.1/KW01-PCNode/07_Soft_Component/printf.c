...
//函数名称：out_char1
//功能概要：通过串口打出一个字符
//参数说明：uartNo:串口号：UART_0、UART_1、UART_2、UART_DEBUG
//       data:数据：通过串口发送的一字节数据
//函数返回：无
//============================================================================

void out_char1(char data) {
	uart_send1(UART_Debug, data);
}
//============================================================================
//函数名称：printk_putc
//功能概要：通过串口打出一个字符
...
int printk(PRINTK_INFO *info, const char *fmt, va_list1 ap)
{
	...
	for (p = (char *) fmt; (c = *p) != 0; p++)
	{
		...
		case 'f':
			...
			{
			un1.a1 = (double ) va_arg1(ap, double);  //返回当前参数，并将ap指向下一个参数
			temp1=un1.tempc.b8;
			un1.c2=un1.c1;
			un1.c1=un1.c1&0x0000;
			un1.tempc.b4=temp1;
			fval=un1.a1;
			}
			else if(length_modifier==1 || length_modifier==2)
			{
				ap=ap+4;
			...
			else
			{
				...
				{
					vlen = printk_mkfloatstr(&fval,vstr,8);//将整型转换成字符型保存在vstr中，返回字符数vlen
					vstrp = &vstr[0];  //指向转换后的第一个字符
				}
				else
				{
...
int myprintf(const char *fmt, ...)
{
	...
	info.func = &out_char1;     //调用的串口输出函数
	va_start1(ap, fmt);  //对ap 进行初始化，让它指向可变参数表里面的第一个参数
